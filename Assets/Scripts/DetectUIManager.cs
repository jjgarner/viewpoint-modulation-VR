...
public class DetectUIManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        currentSelection = 0; // however, selected = false until joystick is pressed
        goalFadeVal = 1;
        goalFillVal = 0;
        prevFadeVal = 0;
        prevFillVal = 0;
        ...
        possibleLocations = new float[] { -0.99f, 0.99f };
        xLocation = transform.position.x;
        yLocation = transform.position.y - 1;
        zLocation = transform.position.z;

        needFadeIn = true;
        selected = false; // for this canvas, indicator is floating between Yes and No, so participant actualy needs to press joystick one way to make a valid selection
    ...
    void Update()
    {
        ...
        else if (confirmed)
        {
            ...
            if (fadeOutTimer >= 1)
            {
                ...
                expManager_ref.GetComponent<ExperimentManager>().detectionResponse[trialNumb] = currentSelection;
                // change flag in exp manager to progress through exp
                expManager_ref.GetComponent<ExperimentManager>().needVectPrefab = true;
